...
class LeNet5DoubleFlatten(BayesianModel):
    def __init__(self, prior_info):
        super(LeNet5DoubleFlatten, self).__init__()
        self.prior = LeNet5DoublePrior(prior_info)
        conv1_prior, conv2_prior, fc1_prior, fc2_prior = self.prior()

        self.conv1 = DoubleRadialConv2dFlatten(in_channels=1, out_channels=20, kernel_size=5, bias=True, prior=conv1_prior, with_global=True)
        self.pool1 = nn.MaxPool2d(kernel_size=2)
        self.conv2 = DoubleRadialConv2dFlatten(in_channels=20, out_channels=50, kernel_size=5, bias=True, prior=conv2_prior, with_global=True)
        self.pool2 = nn.MaxPool2d(kernel_size=2)
        self.fc1 = DoubleRadialLinear(in_features=800, out_features=500, bias=True, prior=fc1_prior, with_global=True)
        self.fc1_nonlinear = nn.ReLU()
        self.fc2 = DoubleRadialLinear(in_features=500, out_features=10, bias=True, prior=fc2_prior, with_global=True)
...
if __name__ == '__main__':
    model = DoubleRadialConv2dFlatten(prior_info='HalfCauchy')
